...
public:
    min_heap(){this->size = 0; this->arr = array_list<T>();};
    min_heap(int size){this->size = size; this->arr = array_list<T>(size, 2*size);}; 
    min_heap(int size, array_list<T> arr){this->size = size; this->arr = arr;};
    T peek();   
    T pop();
    int find(T node);
    void insert(T node);
...
private:
    int size;
    array_list<T> arr;
    void min_heapify(bool down=false);
};

template <typename T> void min_heap<T>::min_heapify(bool down){
    if(!down){
        int curr = size-1;
...
template <typename T> T min_heap<T>::peek(){
    if(size > 0){
        return arr.at(0);
    }
    return T();
...
template <typename T> T min_heap<T>::pop(){
    if(size > 0){
...
template <typename T> void min_heap<T>::insert(T node){
    arr.set(size, node);
    size++;
    min_heapify();
}
...
template <typename T> int min_heap<T>::find(T node){
    return arr.find(node);
}
    
template <typename T> void min_heap<T>::clear(){
    size = 0;
    arr.clear();
}

template <typename T> void min_heap<T>::print(){
    arr.print(size);
}
...
int main(){
    min_heap<int> int_heap =  min_heap<int>();
    int_heap.insert(1);
    int_heap.min_heap::print();
    int_heap.insert(2);
    int_heap.min_heap::print();
    int_heap.insert(3);
    int_heap.min_heap::print();
    ...
    cout << "-------new heap-------" << endl;
    int n[5] = {1,2,3,4,5}; 
    min_heap<int> int_heap2 = min_heap<int>(5, array_list<int>(5, 10, n));
    int_heap2.print();
    ...
    min_heap<test_struct> test_heap = min_heap<test_struct>(); 
